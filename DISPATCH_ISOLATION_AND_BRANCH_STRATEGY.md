...
Status: PROPOSAL ONLY. Nothing in this document has been implemented. No branches, worktrees, or merges should be created/altered based on this doc without explicit sign-off from Sam.
...
## Section 4 — `main` vs `test-gate-fix`: recommendation (restated for clarity)

See Section 1 for full evidence. Restated as an action item:

1. Fast-forward local `main` to include `origin/main`'s one extra commit (`001656f`), then push the reconciled `main` back to `origin` so the 25 local-only commits are no longer sitting unpushed on one machine.
2. Rebase `test-gate-fix` onto the reconciled `main` (`git rebase main` while on `test-gate-fix`, from a dedicated worktree, resolving conflicts once).
3. Use the *post-rebase* `test-gate-fix` tip as the new `BASE_SHA` for the next dispatch round (Section 3), so Phase 2A/2B/2C's remaining/future work is built on top of the reconciled history rather than the old pre-`main`-merge base.
4. This is a one-time reconciliation, not a recurring sync — the evidence found here shows no live process is advancing `main` independently, so no ongoing "coordinate with main every round" policy is needed once step 2 is done.

**This entire section requires Sam's explicit sign-off before execution** — it rewrites `test-gate-fix`'s history via rebase and pushes to `origin/main`, both of which are exactly the kind of action this investigation was scoped to propose, not perform.
