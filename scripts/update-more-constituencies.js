...
// List of constituencies to update
const constituenciesToUpdate = [
  "Limerick City",
  "Limerick County", 
  "Clare",
  "Kerry",
...
/**
 * Updates the selected constituency with data from Wikipedia
...
async function updateConstituencyWithWikipedia(constituencyName) {
  try {
    ...
    console.log(`Starting to update ${constituencyName}...`);
    ...
    console.error(`Error updating ${constituencyName}:`, error);
  }
}

/**
 * Updates multiple constituencies with data from Wikipedia
 */
async function updateMultipleConstituencies() {
  try {
    console.log(`Starting update for ${constituenciesToUpdate.length} constituencies`);
    
    // Process each constituency sequentially to avoid rate limiting
    for (let i = 0; i < constituenciesToUpdate.length; i++) {
      const constituencyName = constituenciesToUpdate[i];
      console.log(`Processing ${i+1}/${constituenciesToUpdate.length}: ${constituencyName}`);
      
      try {
        await updateConstituencyWithWikipedia(constituencyName);
        // Add a small delay between constituencies to avoid rate limiting
        if (i < constituenciesToUpdate.length - 1) {
          console.log('Pausing before next constituency...');
          await new Promise(resolve => setTimeout(resolve, 2000));
        }
      } catch (error) {
        console.error(`Failed to update ${constituencyName}:`, error);
      }
    }
    
    console.log('All constituencies updated successfully!');
  } catch (error) {
