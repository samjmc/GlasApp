...
// Get list of all Irish constituencies
...
/**
 * Updates all constituencies with data from Wikipedia, processing them sequentially
 * with a delay to avoid rate limiting
 */
async function updateAllConstituenciesWithWikipedia() {
  try {
    // Get the file path
    ...
    console.log(`Starting to update all ${allConstituencies.length} constituencies...`);
    
    // Process each constituency with a delay between requests
    for (let i = 0; i < allConstituencies.length; i++) {
      const constituencyName = allConstituencies[i];
      console.log(`Processing ${i+1}/${allConstituencies.length}: ${constituencyName}`);
...
updateAllConstituenciesWithWikipedia()
  .then(() => {
    console.log('Done!');
    process.exit(0);
  })
