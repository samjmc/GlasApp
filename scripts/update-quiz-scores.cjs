...
for (let i = 0; i < lines.length; i++) {
  ...
  // Detect dimension scores - only count as relevant if score is significant (|score| >= 3)
  // or if it's the highest absolute score for that question
  ...
  if (dimMatch && currentQuestionId) {
    ...
    if (Math.abs(score) >= 3) { // Only count significant scores
