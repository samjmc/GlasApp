...
export function NewsArticleCard({ article }: { article: NewsArticle }) {
  ...
  return (
    <Card className="mx-auto w-full max-w-[380px] overflow-hidden transition-shadow hover:shadow-lg sm:max-w-none relative h-[600px] flex flex-col border-0 group">
      ...
      {/* Content Layer */}
      <div className="relative z-10 flex-1 overflow-hidden flex flex-col h-full">
        <AnimatePresence initial={false} mode="wait">
          {!showPolicyVote ? (
            ...
            >
              ...
              <div className="flex-1 p-6 overflow-y-auto no-scrollbar">
                ...
                <div className="flex items-center gap-2 mb-4">
                  {article.sourceLogoUrl && !logoLoadError ? (
                    <div className="h-6 w-6 rounded-full overflow-hidden border border-white/20 bg-white">
                      <img 
                  ...
                  <span className="text-xs font-semibold text-white shadow-black/50 drop-shadow-sm">{article.source}</span>
                  <span className="text-xs text-gray-300">• {formatTimeAgo(article.publishedDate)}</span>
                  
                  {article.storyType && (
                    <Badge variant="outline" className="ml-auto text-[10px] h-5 font-normal capitalize text-gray-200 border-white/20 bg-white/5">
                ...
                {/* Title */}
                <h2 className="text-xl font-bold leading-tight mb-4 text-white drop-shadow-md">
                  {article.title}
                </h2>

                {/* Summary */}
                <div className="mb-6 rounded-xl bg-white/10 p-4 border border-white/10 backdrop-blur-sm">
                  <div className="flex items-center gap-1.5 mb-2">
                    <Sparkles className="w-3.5 h-3.5 text-purple-400" />
                    <span className="text-[10px] font-bold uppercase tracking-wider text-purple-300">AI Summary</span>
                  </div>
                ...
                {((article.affectedTDs && article.affectedTDs.length > 0) || article.politicianName) && (
                  <div className="mb-4">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs font-semibold uppercase tracking-wide text-gray-400">Impact Analysis</span>
                    </div>
                    
                    <div className="flex flex-wrap gap-2">
                      {article.affectedTDs && article.affectedTDs.length > 0 ? (
                        article.affectedTDs.map((td, idx) => (
                          <div key={idx} className="flex items-center gap-2 bg-white/10 rounded-full px-3 py-1.5 border border-white/10 backdrop-blur-sm">
                            <span className="text-xs font-medium text-white">{td.name}</span>
                            <span className={`text-xs font-bold ${td.impactScore >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                        ...
                        <div className="flex items-center gap-2 bg-white/10 rounded-full px-3 py-1.5 border border-white/10 backdrop-blur-sm">
                          <span className="text-xs font-medium text-white">{article.politicianName}</span>
                          <span className={`text-xs font-bold ${article.impactScore && article.impactScore >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                ...
                {article.policyVote && (
                  <div className="mt-auto pt-4">
                    <Button 
                      ...
                      className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-semibold shadow-lg shadow-emerald-900/20 border border-emerald-500/50"
                    >
              ...
              {/* Footer Actions */}
              <div className="border-t border-white/10 p-3 flex items-center justify-around bg-black/40 backdrop-blur-md">
                <Button variant="ghost" size="sm" onClick={handleLike} className={liked ? "text-rose-500 hover:text-rose-400" : "text-gray-300 hover:text-white hover:bg-white/10"}>
                  <ThumbsUp className={`w-4 h-4 mr-1.5 ${liked ? "fill-current" : ""}`} />
                  <span className="text-xs">{article.likes + (liked ? 1 : 0)}</span>
                </Button>
                <Button 
                  variant="ghost" 
                  size="sm" 
                ...
                <Button variant="ghost" size="sm" onClick={handleShare} className="text-gray-300 hover:text-white hover:bg-white/10">
