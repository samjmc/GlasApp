...
/**
 * Turn the panel's consensus into ELO movement for one TD.
 *
 * Pure apart from the repository write: the caller supplies the TD's current ratings and
 * gets back what changed. Credibility = source credibility × panel confidence.
 */
export async function applyPanelResult(
  result: MultiAgentAnalysis,
  article: { id: number; publishedDate: Date | null; credibility: number },
  tdId: number,
  current: EloRatings,
): Promise<EloChange[]> {
  const consensus = result.consensusScores;
  const impacts: ArticleImpacts = {
    overall: consensus.overallImpact,
    transparency: processScoreToImpact(consensus.transparencyScore),
    effectiveness: processScoreToImpact(consensus.effectivenessScore),
    integrity: processScoreToImpact(consensus.integrityScore),
    consistency: processScoreToImpact(consensus.consistencyScore),
  };
  const credibility = article.credibility * consensus.confidence;
  const ageDays = article.publishedDate ? articleAgeDays(article.publishedDate) : 0;

  const { updated, changes } = applyArticle(current, impacts, credibility, ageDays);
  if (changes.length === 0) return changes;

  await repo.applyElo(tdId, updated, changes, {
    articleId: article.id,
...
export async function applyIdeologyToProfile(
  ...
  const { TDIdeologyProfileService } = await import('../services/tdIdeologyProfileService');
...
/** Run the full panel for one (article, TD) pair. */
export const runPanel = runMultiAgentScoring;
