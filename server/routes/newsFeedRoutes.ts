...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const pageLimit = Number.isFinite(parsedLimit) ? Math.max(0, parsedLimit) : 20;
    const pageOffset = Number.isFinite(parsedOffset) ? Math.max(0, parsedOffset) : 0;
      ...
      try {
        ...
        if (sort === 'score' || sort === 'highest') {
          ...
          if (allError) {
            ...
            const impactArticles = articlesWithTotalImpact
            ...
            articles = impactArticles.slice(pageOffset, pageOffset + pageLimit);
            count = impactArticles.length;
