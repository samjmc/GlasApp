...
import { getCachedOrFetch, CACHE_KEYS, CACHE_TTL } from '../../utils/serverCache.js';
 ...
 */
router.get('/constituencies/summary', asyncHandler(async (req, res) => {
  if (!supabaseDb) {
  ...
  // Use cache for constituency summary (15 minute TTL - rarely changes)
  const summaryData = await getCachedOrFetch(
  ...
  res.json(formatSuccess({
    constituencies: summaryData.constituencies,
    totalConstituencies: summaryData.totalConstituencies,
    totalTDs: summaryData.totalTDs
  }));
}));
...
router.get('/constituencies', asyncHandler(async (req, res) => {
  ...
  // Use cache for constituencies list (1 hour TTL - very rarely changes)
 ...
 */
router.get('/constituency/:name', asyncHandler(async (req, res) => {
  ...
  if (!tds || tds.length === 0) {
    return res.status(404).json(
      formatError('ENTITY_NOT_FOUND', `Constituency "${decodedName}" not found`)
    );
  }
  ...
  res.json(formatSuccess(constituency, { entity: 'constituency' }));
}));
